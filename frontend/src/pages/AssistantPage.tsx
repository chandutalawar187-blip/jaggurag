...
export default function AssistantPage({
  ...
  useEffect(() => {
    ...
    const loadConversation = async () => {
      try {
        ...
        if (!cancelled && conv && conv.messages) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // Refresh the sidebar before selecting a newly auto-created conversation.
      // Otherwise reconciliation can briefly see the old list and clear the ID.
      await onConversationUpdated?.();
    ...
    <div className="relative flex-1 flex flex-col h-full bg-[#E4E2DD] text-[#1E1E1E] overflow-hidden slide-up">
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <p className="font-mono text-[9px] opacity-80 mt-0.5">
                      {m.provider === 'ollama' ? 'LOCAL ONLY' : (m.available ? 'READY' : 'OFFLINE')}
                    </p>
                  </div>
                </button>
              ))}
      ...
      <div className="relative z-10 px-6 py-4 border-t-2 border-[#1E1E1E] bg-[#E4E2DD]">
        <div className="max-w-3xl mx-auto">
          <form onSubmit={(e) => { e.preventDefault(); e.stopPropagation(); void handleSend(); }}
      ...
      <div className={`max-w-[85%] ${isUser ? 'order-1' : ''}`}>
        ...
        {res && (
          <div className="mt-2 flex flex-wrap items-center gap-2 text-[10px] font-mono font-bold">
            ...
            {res.sources.length > 0 && (
              <span className="flex items-center gap-1 bg-[#1E1E1E] text-white px-2 py-0.5 border border-[#1E1E1E]">
        ...
        {res?.sources && res.sources.length > 0 && (
          <div className="mt-2 space-y-1.5">
            <div className="border-2 border-[#1E1E1E] bg-[#FFF4E8] px-3 py-2 text-[10px] font-mono font-bold">
              SOURCE EVIDENCE • ANSWER CONFIDENCE: {Math.round(res.confidence * 100)}%
            </div>
