...
async def get_document_content(doc_id: str):
    """Serve a stored document for the in-app viewer."""
    doc = get_document_by_id(doc_id)
    if not doc:
        raise HTTPException(status_code=404, detail="Document not found")

    file_path = Path(doc.get("file_path", "")).resolve()
    allowed_roots = [Path(settings.DATASET_PATH).resolve(), Path(settings.UPLOAD_PATH).resolve()]
    ...
    return FileResponse(
        file_path,
        media_type=doc.get("mime_type") or None,
