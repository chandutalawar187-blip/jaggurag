...
def init_conversation_db() -> None:
    ...
    with get_connection() as conn:
        ...
        try:
            cursor.execute("ALTER TABLE messages ADD COLUMN sources_json TEXT NOT NULL DEFAULT '[]'")
...
def get_conversation(conversation_id: str) -> Optional[Dict[str, Any]]:
    ...
    with get_connection() as conn:
        ...
        conv = dict(conv_row)
        cursor.execute("""
            SELECT id, conversation_id, role, content, model, answer_type, confidence, sources_json, created_at
            FROM messages
...
def add_message(
    ...
    with get_connection() as conn:
        ...
        cursor.execute("""
            INSERT INTO messages (id, conversation_id, role, content, model, answer_type, confidence, sources_json, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (msg_id, conversation_id, role, content, model, answer_type, confidence, json.dumps(sources or []), now))
        conn.commit()
...
def get_recent_messages(conversation_id: str, limit: int = 10) -> List[Dict[str, Any]]:
    ...
    with get_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("""
            SELECT role, content, answer_type
            FROM (
                SELECT role, content, answer_type, created_at
                FROM messages
                WHERE conversation_id = ?
