...
from app.models.database import (
    ...
    get_document_by_path, insert_document, insert_fts_chunks, list_documents,
    insert_document_chunks, has_sqlite_document_chunks,
    update_document,
)
...
def process_file(filepath: str | Path, force: bool = False) -> Optional[str]:
    ...
    if (
        ...
        and not force
        and has_sqlite_document_chunks(existing["id"])
    ):
    ...
    try:
        ...
        insert_document_chunks(all_chunks)
...
def synchronize_dataset() -> int:
    """Reconcile the filesystem and search indexes before a live query.

    The watcher handles normal changes asynchronously, but a query must never
    depend on whether that background event has completed.
    """
    dataset_dir = Path(settings.DATASET_PATH)
    ...
    with _sync_lock:
        ...
        for filepath in dataset_dir.iterdir():
            if not filepath.is_file() or filepath.name.startswith("."):
                continue
            if filepath.suffix.lower() not in EXTENSION_TO_TYPE:
                continue
            path = str(filepath)
        ...
        for path, document in indexed.items():
            if path not in current_paths:
                _remove_document_data(document["id"])
                from app.models.database import delete_document
                delete_document(document["id"])
                changed += 1
                log.info("Live sync removed missing file: %s", document["filename"])

        if changed:
            log.info("Live dataset sync completed: %d document changes", changed)
        return changed
